                ...
                <div className="max-h-[520px] overflow-auto">
                  {sortedOneway.map((f) => (
                    <div key={f.id} className="grid grid-cols-[1fr_auto_auto] items-center gap-3 border-b px-3 py-2 text-sm hover:bg-slate-50">
                      <div>
                        <div className="font-semibold">{hhmm(f.departure.time)} - {hhmm(f.arrival.time)}</div>
                        <div className="text-xs text-slate-600">{f.airline} {f.flightNumber} · {f.stops === 0 ? 'Bay thẳng' : `${f.stops} điểm dừng`} · {durationText(f.duration)}</div>
                      </div>
                      <div className="font-semibold">{fmt(f.price.amount)}</div>
                      <button className={`rounded-full px-2 py-1 text-xs font-semibold text-white ${airlineClass(f.airline)}`}>Nguồn flyclaw</button>
                    </div>
                  ))}
                  {!loading && sortedOneway.length === 0 && !error && <div className="p-3 text-sm text-slate-500">Không có dữ liệu.</div>}
                </div>
              </div>
            </div>
          )}
