      ...
      <div>
        ...
        {airlines.map(a => (
          <label key={a.name} className="block">
            <input type="checkbox" checked={airlineFilter.has(a.name)} onChange={() => toggle(airlineFilter, a.name, setAirlineFilter)} /> {a.name} — từ {a.minPrice.toLocaleString('vi-VN')}đ
          </label>
        ))}
      </div>
    </div>
