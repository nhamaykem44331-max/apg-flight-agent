...
import { FlightResult, SearchPayload, SearchResponse } from '@/lib/types';
...
export default function SearchPage() {
  ...
  const payload: SearchPayload = {
    ...
    cabin: (sp.get('cabin') as any) || 'economy',
    tripType: (sp.get('tripType') as any) || 'oneway',
  };
  ...
  return (
    <main className="mx-auto max-w-6xl px-4 py-8">
      <div className="mb-4 rounded-2xl border bg-white p-4">
        <div className="font-bold">{payload.from} → {payload.to} | {payload.date} | {payload.adults} người lớn <button className="ml-2 text-brand underline" onClick={()=>history.back()}>Sửa</button></div>
      </div>

      {loading && <LoadingSkeleton />}
      {error && (
        <div className="rounded-2xl border border-red-200 bg-red-50 p-4 text-red-700">
          {error} <button className="underline" onClick={()=>location.reload()}>Thử lại</button>
        </div>
      )}

      {!loading && !error && data && (
        <div className="grid gap-4 md:grid-cols-[250px_1fr]">
          <FilterSidebar flights={data.results} stopFilter={stopFilter} setStopFilter={setStopFilter} airlineFilter={airlineFilter} setAirlineFilter={setAirlineFilter} />
          <div>
